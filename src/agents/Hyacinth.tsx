...
export interface Hyacinth {
  ...
  age: number; // Age in seconds (dies at 40)
}
...
interface HyacinthSpriteProps {
  ...
  onAgeChange: (id: string, newAge: number) => void;
}
...
export const HyacinthSprite = ({
  ...
  const lastAgeUpdateRef = useRef<number>(0); // For age tracking
  ...
  useTick((ticker) => {
    ...
    if (lastGrowthUpdateRef.current >= 1.0) {
      ...
      if (hyacinth.biomass < MAX_BIOMASS && river.totalNutrients > 0) {
        ...
        const currentGrowthRate = calculateGrowthRate(
          river.temperature,
          river.sunlight,
          hyacinth.nur,
        );
        ...
        const newBiomass = Math.min(
          hyacinth.biomass + biomassIncrease,
          MAX_BIOMASS,
        );
    ...
    if (lastReproductionCheckRef.current >= 1.0) {
      ...
      if (
        randomValue <= reproductionChance * 100 && // Convert 0-1 to 1-100 for comparison
        ...
        river.totalNutrients > 0
      ) {
        // Cannot reproduce if no nutrients left

        // Check if there's space around the hyacinth for reproduction
        const reproductionPosition = findReproductionSpace(
          hyacinth,
          allHyacinths,
          spriteSize,
        );
        if (reproductionPosition) {
          onReproduction(
            hyacinth.id,
            reproductionPosition.x,
            reproductionPosition.y,
          );
        }
      }
    ...
    // Handle age tracking every second (independent of reproduction)
    lastAgeUpdateRef.current += deltaTime / 60; // Convert to seconds

    if (lastAgeUpdateRef.current >= 1.0) {
      const newAge = hyacinth.age + 1;

      // Check if hyacinth should die of old age
      if (newAge >= 40) {
        onDeath(hyacinth.id);
        return; // Exit early since this hyacinth is now dead
      }

      // Update age using callback
      onAgeChange(hyacinth.id, newAge);

      lastAgeUpdateRef.current = 0;
    }

    // Calculate river flow effect
    const riverFlowX =
      Math.cos(river.flowDirection) * river.flowRate * deltaTime;
    const riverFlowY =
      Math.sin(river.flowDirection) * river.flowRate * deltaTime;
    ...
    for (const otherHyacinth of allHyacinths) {
      ...
      if (distance < comfortDistance && distance > 0) {
        ...
        const repulsionStrength =
          (comfortDistance - distance) / comfortDistance;
    ...
    if (currentSpeed > maxVelocity) {
      velocityRef.current.vx =
        (velocityRef.current.vx / currentSpeed) * maxVelocity;
      velocityRef.current.vy =
        (velocityRef.current.vy / currentSpeed) * maxVelocity;
    }
    ...
    if (hyacinth.parent !== null) {
      ...
      if (parent) {
        ...
        const connectionDistance =
          Math.max(spriteSize.width, spriteSize.height) * 3;
    ...
    const floatingOffset =
      Math.sin(floatingTimeRef.current + hyacinth.id.length * 0.5) * 2; // Use ID length for phase offset
